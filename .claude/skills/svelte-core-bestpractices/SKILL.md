...
<!-- TODO(references-not-vendored): the upstream skill at sveltejs/ai-tools also ships
     9 files under references/ ($inspect, @attach, @render, await-expressions, bind,
     each, hydratable, snippet, svelte-reactivity). Bulk download from raw.githubusercontent.com
     was denied by the harness on memory-poisoning grounds — reviewer must explicitly
     authorize the source before vendoring. Until then, agents should fetch them on
     demand from the upstream URLs when a deeper dive on a referenced topic is needed. -->
