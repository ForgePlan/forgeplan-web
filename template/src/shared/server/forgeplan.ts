...
const READ_ONLY_SUBCOMMANDS = new Set([
  'list', 'health', 'graph', 'get', 'order', 'blocked', 'claims',
  'stale', 'log', 'score', 'tree', 'blindspots', 'journal'
]);
...
function readWorkspaceRoot(): string {
  ...
  if (existsSync(cfgPath)) {
    try {
      const cfg = JSON.parse(readFileSync(cfgPath, 'utf8')) as { workspaceRoot?: string };
