<script lang="ts">
  ...
  import type { ArtifactDetail, GraphEdge } from '$lib/types';
  import { fetchArtifact } from '$lib/stores/forgeplan';
  import { kindLabel, kindLabelColor, statusRing, reffTone } from '$lib/theme';

  export let id: string;
  export let edges: GraphEdge[] = [];

  const dispatch = createEventDispatcher<{ close: void; navigate: { id: string } }>();
  ...
  async function load(target: string) {
    ...
    error = null;
    try {
      const next = await fetchArtifact(target);
...
<aside class="panel">
  <header>
    <button class="close" type="button" on:click={() => dispatch('close')} aria-label="Close">×</button>
    <div class="hd">
          ...
          <ul>
            {#each outgoing as e (e.from + e.to + e.relation)}
              <li>
                ...
                <button type="button" class="ref" on:click={() => dispatch('navigate', { id: e.to })}>{e.to}</button>
              </li>
            {/each}
          </ul>
        {/if}
        {#if incoming.length}
          ...
          <ul>
            {#each incoming as e (e.from + e.to + e.relation)}
              <li>
                <button type="button" class="ref" on:click={() => dispatch('navigate', { id: e.from })}>{e.from}</button>
                <span class="rel">{e.relation}</span>
              </li>
            {/each}
          </ul>
