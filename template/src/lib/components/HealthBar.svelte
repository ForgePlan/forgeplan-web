<script lang="ts">
  import { healthPoller } from '$lib/stores/forgeplan';

  const { store } = healthPoller;
  $: health = $store.data;
...
<header class="bar">
  ...
  <div class="stats">
    ...
    {:else if $store.error}
      <span class="stat err" title={$store.error}>error</span>
    {/if}
  </div>
  <div class="meta">
    <span class="muted">updated {lastUpdated}</span>
    <span class="pulse" class:on={$store.loading}></span>
  </div>
</header>
