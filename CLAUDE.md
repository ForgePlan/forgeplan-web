...
- `dist/` — pre-built SvelteKit app for the `stable` image (`vite build`
  + esbuild single-file bundle, ~1.8 MB, no `node_modules/`). Generated
  by `scripts/build.mjs`. **Published**, copied verbatim into
  `.forgeplan-web/` by `init`. **Default** source for `init`.
- `dist-nightly/` (and any other `dist-<name>/`) — same bundle shape
  for non-default images. **Published**; copied to `.forgeplan-web/`
  only when `init --image <name>` is passed. v1 ships only `nightly`.
  Image registry: `config/images.json`. Feature-flag registry:
  `config/features.json`. Lifecycle policy: `config/IMAGES.md`. See
...
dist-nightly/          # pre-built artifact for the `nightly` image, same bundle
                       # shape as dist/. Opt-in via `init --image nightly`.
                       # PRD-030 / RFC-026 / ADR-005.
                       # contains: index.js (single ESM bundle), env.js, client/,
                       #           package.json, forgeplan-web-build.json
config/                # image + feature-flag registries (PRD-030)
  images.json          # image name → flags
  features.json        # flag id → metadata (addedIn, expiresIn, owner, rollout)
  IMAGES.md            # maintainer guide
...
`PATH`, (2) copies the package's bundled image (`dist/` for `stable`
by default, `dist-<name>/` when `--image <name>` is passed) →
`.forgeplan-web/` (skipping existing files unless `--force`),
(3) writes `forgeplan-web.json` with `workspaceRoot`, `version`,
`image`, `scope` so the SvelteKit server knows where to spawn
`forgeplan` and `update` knows which image to refresh into.
**No `npm install` runs at user side** — every shipped image is a
self-contained esbuild bundle with all runtime deps inlined (no
`node_modules/`). `start` spawns `node .forgeplan-web/index.js`
