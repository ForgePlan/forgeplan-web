...
async function main() {
  ...
  if (matches1.length !== 1) {
    fail(`gitignore: expected 1 .forgeplan-web entry after first init, got ${matches1.length}`);
  }
  ...
  if (matches2.length !== 1) {
    fail(`gitignore: expected 1 .forgeplan-web entry after second init, got ${matches2.length} (idempotency broken)`);
  }
  log(`gitignore: still ${matches2.length} match after second init (idempotent)`);
  ...
  const health = await check(`http://127.0.0.1:${port}/api/health`).then((r) => r.json());
  if (!health.ok) fail(`/api/health: ${JSON.stringify(health)}`);
  log(`/api/health: ok (project=${health.data?.project ?? '?'})`);

  const list = await check(`http://127.0.0.1:${port}/api/list`).then((r) => r.json());
